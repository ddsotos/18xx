...
緑フェーズ以降のMRでは、小会社が相手を提案し、同意後に新社長が未使用大会社を選んで能力なし合併を実行する。旧2社の
...
列車上限を超えた場合は、上限内になるまで専用DiscardTrain状態でMRを中断する。固有能力はUIにpending表示だけを出し、
ルール効果を適用しないため、Overnightによる完全占有都市の通過など能力由来の接続例外は後続実装とする。

## 未完了

W01の公式各部品台帳、公式固定盤面への差替え、固有能力、
厳密な輸出・破産処理、UI受入を
