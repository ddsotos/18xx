...
## 2026-09-16：alpha完走vertical slice
...
緑フェーズ以降のMRでは、小会社が相手を提案し、同意後に新社長が未使用大会社を選んで合併を実行する。旧2社の
個人・市場・自己保有株を20%→10%へ換算し、合計保有が最多の個人を新社長とする。株価は旧2社の平均以下へ
切り下げ、現金・列車・配置済みhubを移管する。同じ都市のhub重複は1個へ整理する。

合併提案は相手社長の明示的なaccept/rejectを待ち、拒否済みの無順序ペアを同じMRで再提案できない。
候補は通常グラフ上でhub間が遮断なく接続する会社に限り、列車距離は参照しない。合併直後に大会社の
列車上限を超えた場合は、上限内になるまで専用DiscardTrain状態でMRを中断する。

能力は元小会社ID・能力ID・可変stateを持つ共通registryへ移し、合併Actionで大会社へ引き継ぐ。
Adaptiveの設立時ホーム選択に加え、Spaciousの列車上限+1、Overnightの完全占有都市通過グラフと
合併接続例外、Expressの「保有列車が1台なら停車数+1」を実装した。これらは合併先でも同じregistryから有効になる。
Bridging、Agricultural、Tunneling、Eastern Mining、Northern Portは公式タイル・地形データ、
ExpansiveとSuburbanは専用配置Action、Resourcefulは廃車猶予状態とともに後続実装する。

## 未完了

W01の公式各部品台帳、公式固定盤面への差替え、残りの固有能力、
厳密な輸出・破産処理、UI受入を
引き続き実装する。alphaはゲーム一覧から開始・完走できる構造になったが、公式ルール完成版ではない。
...
既存基盤テストはユーザー側で通過済み。今回追加した合併テストの実行はユーザー環境で行う。
