...
module Engine
  describe Game::G1890::Game do
    ...
    describe 'scenario C setup' do
      ...
      it 'allows the Osaka West tile to upgrade to the BOW tile in the brown phase' do
        corporation = game.corporations.first
        round = game.operating_round(1)
        ...
        osaka_west = game.hex_by_id('H11')
        osaka_west.tile.cities.first.place_token(corporation, corporation.next_token, check_tokenable: false)
        ...
        upgrade.rotate!(0)
        expect(track_step.upgradeable_tiles(corporation, osaka_west).map(&:name)).to contain_exactly('BOW')
        expect(track_step.upgradeable_tiles(corporation, osaka_south).map(&:name)).not_to include('BOW')
        expect(game.upgrades_to?(osaka_west.tile, upgrade)).to be(true)
        expect(game.upgrades_to?(osaka_south.tile, upgrade)).to be(false)

        track_step.process_lay_tile(Action::LayTile.new(corporation, hex: osaka_west, tile: upgrade, rotation: 0))

        expect(osaka_west.tile.name).to eq('BOW')
        expect(osaka_west.tile.label.to_s).to eq('OW')
      end

      it 'allows the brown Y tile 831 only from a Y tile' do
        osaka_south = game.hex_by_id('I12')
        osaka_west = game.hex_by_id('H11')
        upgrade = game.tiles.find { |tile| tile.name == '831' }
    ...
    describe 'initial auction' do
      ...
      it 'requires the first bid on a later company to exceed face value by 5' do
        step = game.round.active_step
        kobe_tram = step.companies[1]

        expect(step.min_bid(kobe_tram)).to eq(45)

        step.process_bid(Action::Bid.new(game.current_entity, company: kobe_tram, price: 45))

        expect(step.min_bid(kobe_tram)).to eq(50)
        expect(step.committed_cash(game.players.first)).to eq(45)
      ...
      it 'resolves multiple bids after the earlier company is bought' do
        ...
        step.process_bid(Action::Bid.new(player_a, company: kobe_tram, price: 45))
        step.process_bid(Action::Bid.new(player_b, company: kobe_tram, price: 50))
        step.process_bid(Action::Bid.new(player_c, company: arima, price: 20))

        expect(step.active_entities).to eq([player_a])
        expect(step.min_bid(kobe_tram)).to eq(55)

        step.process_pass(Action::Pass.new(player_a))

        expect(kobe_tram.owner).to eq(player_b)
        expect(player_b.cash).to eq(750)
    ...
    describe 'Kintetsu conversion' do
      ...
      it 'can run a train received from Nara after exchange using game actions' do
        ...
        game.phase.next! until game.phase.name == '4'
        nara_company = game.initial_auction_companies[9]
        nara = game.minors.find { |minor| minor.id == nara_company.id }
        nara_train = game.trains.find { |train| train.name == '3' && train.owner == game.depot }
        game.buy_train(nara, nara_train, :free)
        nara_train.operated = true
        other_corporation = game.corporations.find { |corporation| corporation != kintetsu }
        setup_round.instance_variable_set(:@entities, [other_corporation])
        setup_round.instance_variable_set(:@entity_index, 0)
        setup_round.instance_variable_set(:@current_operator, other_corporation)
        ...
        game.process_action(
          Action::BuyShares.new(nara_company, shares: game.reserved_kintetsu_shares(kintetsu).first(2)),
        )

        expect(nara).to be_closed
        expect(nara_train.owner).to eq(kintetsu)
        expect(nara_train.operated).to be(false)
        ...
        token_city = game
                     ...
                     .find { |city| city.tokens.compact.any? { |token| token.corporation == kintetsu } }
        route_city = game.hex_by_id('H15').tile.cities.first
        route = Route.new(game, game.phase, nara_train)
        [token_city, route_city].each { |node| route.touch_node(node) }
        president_cash = kintetsu.owner.cash
        share_price = kintetsu.share_price
        ...
        expect(nara_train.operated).to be(true)
        expect(kintetsu.owner.cash).to eq(president_cash + 12)
        expect(kintetsu.share_price.price).to be > share_price.price
      end
      ...
      it 'can run a train received from Hantetsu after forced conversion using game actions' do
        ...
        expect(hantetsu).to be_closed
        expect(hantetsu_train.owner).to eq(kintetsu)
        expect(hantetsu_train.operated).to be(false)
        expect(game.kintetsu_special_operating?).to be(true)
        ...
        token_city = game
                     ...
                     .find { |city| city.tokens.compact.any? { |token| token.corporation == kintetsu } }
        route_city = game.hex_by_id('H15').tile.cities.first
        route = Route.new(game, game.phase, hantetsu_train)
        [token_city, route_city].each { |node| route.touch_node(node) }
        president_cash = kintetsu.owner.cash
        share_price = kintetsu.share_price
        ...
        expect(hantetsu_train.operated).to be(true)
        expect(kintetsu.owner.cash).to eq(president_cash + 12)
        expect(kintetsu.share_price.price).to be > share_price.price
        expect(game.kintetsu_special_operating?).to be(false)
      end
      ...
      it 'can run a train received from Nara after the 6 train forced merge using game actions' do
        buy_all_initial_companies(game)
        nara_company = game.initial_auction_companies[9]
        nara = game.minors.find { |minor| minor.id == nara_company.id }
        kintetsu = game.corporations[5]
        ...
        nara_train = game.trains.find { |candidate| candidate.name == '4' && candidate.owner == game.depot }
        game.buy_train(nara, nara_train, :free)
        nara_train.operated = true

        %w[4 5 6].each do |name|
          train = game.trains.find { |candidate| candidate.name == name && candidate.owner == game.depot }
          game.phase.buying_train!(game.corporations.first, train, train.owner)
        end
